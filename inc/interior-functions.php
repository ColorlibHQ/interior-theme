...
// Rating function
function interior_rating_function( $rating = 5 ) {
	$html = '';
	for( $i = 0; $i < $rating; $i++ ) {
...
// Tags function
function interior_get_tax_function( $taxonomy = 'interior-tag' ) {
...
// Tags function
function interior_get_taxonomies( $taxonomy = 'listing_category', $hide_empty = false ) {
	$args = [
		'taxonomy' => $taxonomy,
...
function interior_meta( $id = '', $value_type = true ){
    
    $value = get_post_meta( get_the_ID(), $id, $value_type );
    
    return $value;
}

// interior
function interior_get_styles_from_meta( $footer_bg_opt ) {
	$bg_styles = '';
...
if ( ! function_exists( 'interior_posted_comments' ) ) :
    function interior_posted_comments(){
        ...
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__('No Comments','interior');
...
function interior_social_sharing_buttons( $ulClass = '', $tagLine = '', $show_label = false ) {
...
if( ! function_exists('interior_blog_single_post_navigation') ) {
	function interior_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'interior_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'interior' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'interior' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'interior_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'interior' ), get_comment_author_link() ); ?></h5>
...
function interior_theme_logo( $class = '' ) {
	...
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'interior_site_logo_107x39' );
	...
	if( !empty( $imageUrl[0] ) ){
		...
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( INTERIOR_DIR_IMGS_URI . 'logo.png' ).'" alt="'.esc_attr( get_bloginfo('name') ).'"></a>';
	}
...
function interior_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'interior' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'interior' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
// Themify Icon
function interior_themify_icon(){
    return [
        'seo_1' 	=> __('Search Icon', 'interior'),
        'seo_2' 	=> __('Mike Icon', 'interior'),
        'seo_3' 	=> __('Share Icon', 'interior'),
    ];
}

// CTA section
function interior_get_cta_area() {
	$cta_section_title      = !empty( interior_opt( 'interior_cta_section_title') ) ? interior_opt( 'interior_cta_section_title') : '';
	$cta_section_text       = !empty( interior_opt( 'interior_cta_section_text') ) ? interior_opt( 'interior_cta_section_text') : '';
	$cta_section_btn_label  = !empty( interior_opt( 'interior_cta_section_btn_label') ) ? interior_opt( 'interior_cta_section_btn_label') : '';
	$cta_section_btn_url    = !empty( interior_opt( 'interior_cta_section_btn_url') ) ? interior_opt( 'interior_cta_section_btn_url') : '';
	?>
    <!-- cta-area-start -->
...
function interior_get_contact_details_area() {
	?>
    <!-- location-area-start -->
    <div class="addres-area black-bg section-padding">
        <div class="container">
            <div class="row">
                <?php
				// Contact items
				$contact_items = interior_opt('interior_contact_details_section_fields');
                if ( $contact_items == true ) {
                    ...
                    if( is_array( $contact_items ) && count( $contact_items ) > 0 ){
                        foreach ($contact_items as $value) {
                            switch ($counter) {
                                case $counter == 2:
                                    $icon_img = INTERIOR_DIR_ICON_IMG_URI . 'clock.png';
                                    break;
                                case $counter == 3:
                                    $icon_img = INTERIOR_DIR_ICON_IMG_URI . 'envelope.png';
                                    break;
                                default:
                                    $icon_img = INTERIOR_DIR_ICON_IMG_URI . 'globe.png';
                                    break;
                            }
                            $counter++;
