...
if ( ! function_exists( 'interior_breadcrumbs' ) ) {
    function interior_breadcrumbs( $args = array() ) {
        ...
        */
        ...
        $html .= '<li class="breadcrumb-item"><a class="bread-link s-text16 bread-home" href="' . esc_url( get_home_url('/') ) . '" title="' . esc_attr( $args['home_title'] ) . '">'.esc_html__( 'Home', 'interior' ).' </a></li>';
        // Post
        if ( is_singular( 'post' ) ) {
            ...
            $html .= '<li class="breadcrumb-item active" aria-current="page">'.esc_html__('Search results for:','interior') . get_search_query() . '</li>';
        } elseif ( is_404() ) {
            $html .= '<li class="breadcrumb-item active" aria-current="page">' . esc_html__( 'Error 404', 'interior' ) . '</li>';
        } elseif ( is_home() ) {
