...
function interior_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'interior' ) . '</span></li>';
...
// Blog categories
function interior_featured_post_cat(){
...
// social media
if ( ! function_exists( 'interior_social' ) ) {
	function interior_social( $args = array() ) {
		...
		$url = interior_opt( 'interior_social_url' );
...
// header cart count
function interior_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( INTERIOR_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'interior' ); ?>">
